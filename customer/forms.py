...
class CustomerEditForm(forms.ModelForm):
	...
	#blacklisted = 
	#blacklist = forms.MultipleChoiceField(label='Checkbox For', required=False, widget = forms.CheckboxSelectMultiple, choices={"1":1, "2":2, "3":3})
	#blacklist = forms.MultipleChoiceField(label='Select blacklisted products', required=False, widget = forms.CheckboxSelectMultiple, choices=Product.objects.all().order_by('product_name').values('id', 'product_name'))
	#CHOICES = {}
	#blacklist = forms.MultipleChoiceField(label='Select blacklisted products', required=False, widget = forms.CheckboxSelectMultiple, choices=CHOICES)
	#is_blacklisted = form.BooleanField()
	...
	def __init__(self, *args, **kwargs):
		...
		self.fields['blacklist'].initial = [c.pk for c in Customer.objects.get(pk=customer).blacklist.all()]
	
	class Meta:
		model = Customer
		fields = ['blacklist']
		widgets = { 'blacklist': MyCheckboxSelectMultiple, }
